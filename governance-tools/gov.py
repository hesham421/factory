...
from toolkit.common import blocks, counts_line, now_iso, read_json, rel, rel_to, write_json   # noqa: E402
...
def _is_submodule_of(root: Path, parent: Path) -> bool:
    ...
    else `project.checkout_default`) as an independent, gitignored clone — nested
    in the filesystem, unrelated in git. `CFG.root in root.parents` alone cannot
...
def _source_label(src: Path) -> str:
    """How a fetched input NAMES the publication it came from.

    Absolute, this is `/Users/<whoever>/<wherever>/…` — the one machine that ran
    the fetch. It is written into a header and a meta file that are then
    committed and delivered to every consumer, where it resolves to nothing and
    cannot be compared against anything. Relative to the project checkout it is
    the same string on every machine, so a stale input is provable by diff."""
    return rel_to(src, CFG.project_checkout())
...
def _record_input(name: str, mod: str, version: int, host: str, checkout: Path, src: Path, dst: Path) -> dict:
    ...
    meta = {"input": name, "repo": host, "commit": _head_sha(checkout),
            "source": _source_label(src), "file": dst.name,
