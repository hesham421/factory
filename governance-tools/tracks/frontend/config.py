...
import os as _os
REPO_BASE_PATH = Path(_os.environ["GOV_FACTORY_ROOT"]) if _os.environ.get("GOV_FACTORY_ROOT") \
...
# In the factory both tracks live side by side: read the backend TRACK's
# registry directly (still read-only, still the single module-identity truth).
_BACKEND_TRACK_REGISTRY = Path(__file__).resolve().parent.parent / "backend" / "modules-registry.json"
BACKEND_REGISTRY_FILE = _BACKEND_TRACK_REGISTRY if _BACKEND_TRACK_REGISTRY.parent.exists() \
    else REPO_BASE_PATH.parent.parent / "shared" / "modules-registry.json"
