---
name: P3.2
description: Frontend Execution Plan engine of the governance factory (analysis pass 2). Use when running `/analyze-pass2` reaches this stage, or to (re)generate this stage for a module version.
---

# P3.2 — Frontend Execution Plan
...
```
PASS      : 2          (see governance-tools/config.py PASS_1 / PASS_2)
INPUTS    : modules/[MOD]/vN/_inputs/api-docs-{mod}.md + ui-shell-manifest-{mod}.md (pass-2 gate) · flow-diagram · ui-ux-spec · registries
PRODUCES  : frontend-execution-plan-{mod}.md + registry-exec-fe-{mod}.md  → split (frontend) → DELIVER
NEXT      : — end of analysis
...
3. `git add` + commit: `"P3.2: [MOD] v[N] — <one line>"`. The commit IS the
   ledger row; no JSON ledger.
4. If REVIEW = YES → stop and run `/review-gate P3.2` before the next stage.
