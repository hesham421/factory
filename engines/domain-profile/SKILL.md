---
name: domain-profile
description: Domain Profile engine of the governance factory (analysis pass 1). Use when running `/analyze-pass1` reaches this stage, or to (re)generate this stage for a module version.
---
...
INPUTS    : the platform brief / existing domain knowledge
PRODUCES  : domain/domain-profile.md (once per platform) + per-module inherit section
NEXT      : P0
REVIEW    : no (covered by the next station)
...
3. `git add` + commit: `"domain-profile: [MOD] v[N] — <one line>"`. The commit IS the
   ledger row; no JSON ledger.
4. If REVIEW = YES → stop and run `/review-gate domain-profile` before the next stage.
