---
name: P2.5
description: UI/UX Design engine of the governance factory (analysis pass 1). Use when running `/analyze-pass1` reaches this stage, or to (re)generate this stage for a module version.
---
...
INPUTS    : srs-{mod}.md + db-script-{mod}.md
PRODUCES  : flow-diagram-{mod}.md · ui-ux-spec-{mod}.md
NEXT      : P3.1
...
3. `git add` + commit: `"P2.5: [MOD] v[N] — <one line>"`. The commit IS the
   ledger row; no JSON ledger.
4. If REVIEW = YES → stop and run `/review-gate P2.5` before the next stage.
