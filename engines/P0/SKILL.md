---
name: P0
description: Platform Inception engine of the governance factory (analysis pass 1). Use when running `/analyze-pass1` reaches this stage, or to (re)generate this stage for a module version.
---
...
INPUTS    : domain/domain-profile.md
...
3. `git add` + commit: `"P0: [MOD] v[N] — <one line>"`. The commit IS the
   ledger row; no JSON ledger.
4. If REVIEW = YES → stop and run `/review-gate P0` before the next stage.
