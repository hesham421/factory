---
name: P1
description: SRS engine of the governance factory (analysis pass 1). Use when running `/analyze-pass1` reaches this stage, or to (re)generate this stage for a module version.
---
...
INPUTS    : prd-{mod}.md + P0 outputs
PRODUCES  : srs-{mod}.md + registry-srs-{mod}.md
NEXT      : P2
...
3. `git add` + commit: `"P1: [MOD] v[N] — <one line>"`. The commit IS the
   ledger row; no JSON ledger.
4. If REVIEW = YES → stop and run `/review-gate P1` before the next stage.
