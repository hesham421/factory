---
name: P2
description: Database engine of the governance factory (analysis pass 1). Use when running `/analyze-pass1` reaches this stage, or to (re)generate this stage for a module version.
---
...
INPUTS    : srs-{mod}.md
PRODUCES  : db-script-{mod}.md + registry-db-{mod}.md
NEXT      : P2.5
...
3. `git add` + commit: `"P2: [MOD] v[N] — <one line>"`. The commit IS the
   ledger row; no JSON ledger.
4. If REVIEW = YES → stop and run `/review-gate P2` before the next stage.
