...
### الرؤية — لم تكن مفروضة، وصارت

كان هنا إقرارٌ بأن الـ submodule يعطي كل مستهلك المستودع كاملاً، وأن `CODEOWNERS`
يفرض **الكتابة** لا الرؤية. ذلك صحيح عن git، وغير كافٍ: الشجرة الخطأ أمام أحدهم
هي **بالضبط** كيف حُذفت حوكمة ستة موديولات من جلسة فرونت.

الرافعة الوحيدة على القراءة هي `sparse-checkout`، وأنماطها **مشتقّة** من نفس
الإعلانات لا مكتوبة:

| | يراه |
|---|---|
| مرحلة تسمّي مساراً (`P3_1` · `P3_2`) | ذلك المسار وحده |
| مرحلة لا تسمّي أحداً (`P0…P2`) | **الجميع** — هي التحليل الذي يُنفَّذ عنه |
| حزم المسار | مساره |
| قسم له `writer` | كاتبه، ما لم يُعلَن `readers` أوسع |
| `api-docs` | `readers: all` — العقد المنشور: الباك يكتبه والفرونت يبني عليه |

```bash
gov.py sparse --track backend                      # اطبع
gov.py sparse --track backend --apply-to ../backend/governance/shared
./scripts/governance pull                          # يعيد التضييق تلقائياً
```

> **التضييق ليس عزلاً.** البايتات تظلّ في `.git`، وللمستهلك أن يوسّع نفسه.
> ما يشتريه أن الشجرة الخطأ لا تقع أمام أحد **بالصدفة** — وهي كل ما حدث.

وتحقّقت أنه آمن قبل تبنّيه: المخفيّ يبقى في الفهرس، و`git add -A` بعد كتابة في
القسم المسموح سجّل ملفاً واحداً و**صفر حذف**.
...
## ٥-٦ · الاتجاه العكسي: ما يكتشفه التنفيذ

التصميم حتى الآن يصف اتجاهاً واحداً — المصنع يكتب والطرفان يقرآن. والاتجاه
الآخر موجود فعلاً وكان مهمَلاً: **المنفِّذ يكتشف أثناء البناء ما لم تستطع الخطة
معرفته** — نقطة نهاية لم تُنشَر، حقل باسمين، تبعية بلا نظير.

### القناة

الطرف يسجّل في **قسمه هو** (فقاعدة الكاتب الواحد محفوظة)، والمصنع **يقرأ ولا
يكتب**:

```
modules/{MOD}/{track}/execution-state.json
    api_doc_gaps[]  ·  blocked[]  ·  deferred_xm[]
        { type · phase · sub · endpoint · detail · resolution }
```

`feedback:` في `factory.yaml` يعلن القنوات **ومفردات الحالة**. وهذه الأخيرة هي
ما كان ناقصاً: `resolution` نثر يكتبه منفِّذ، وكان يُكتب بإحدى عشرة صيغة. القارئ
يأخذ الكلمة الأولى بلا حساسية لحالة أو ترقيم، وما ليس في القوائم المعلَنة يعود
**`UNRECOGNISED` لا مُخمَّناً**.

> فجوة تُصنَّف مغلقةً بالخطأ أسوأ من فجوة لا تُصنَّف أصلاً.

### القراءة والمنع

| | |
|---|---|
| `gov.py feedback [-m MOD]` | ما سجّله الطرفان ولم يُجَب عنه |
| `gates[*].requires_feedback: answered` | البوابة **لا تُفتح** وفيها عنصر مفتوح |
| `gov.py waive-feedback --by --why` | قرار بشري مسمّى يفتحها |

**التجاوز مثبَّت بالعناصر التي رآها** — بنفس ما يفعله `approve` حين يربط
الموافقة بـ`artifact_sha`. فجوة تُسجَّل بعده **لا يغطّيها**، والبوابة تُغلق من
جديد. لأن تجاوزاً دائماً ليس قراراً بل **مفتاح إطفاء**، والمفتاح هو ما يمدّ
الناس أيديهم إليه حين تسأل بوابةٌ سؤالاً لا جواب له.

> قيس على الواقع عند البناء: **٦٩ عنصراً** سجّلها الطرفان عبر ستّة موديولات،
> **٣٨ منها بلا جواب** — ولم يكن في `governance-tools` سطر واحد يفتح الملف.
