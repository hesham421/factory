# PASS 1 — module DEMO v1 — bundled session (5 stages, one commit per stage)
...
==============================================================================
# BRIEF — stage `P0.5` (PRD) · module DEMO · v1 · profile `erp`
...
```
══════════════════════════════════════════════════════════════════
PRD EXTRACTION REPORT — DEMO — [date]
══════════════════════════════════════════════════════════════════
STORIES DRAFTED
  + US-DEMO-001 — [one line] — Traces: [ids] — Source: [ref]
...
event history    : "P0.5 completed: DEMO v1 — [N] stories, [N] decisions"
```
...
==============================================================================
# BRIEF — stage `P2` (Database) · module DEMO · v1 · profile `erp`
...
- Read only what this brief contains (generated current state); never open version folders yourself.
- Write exactly these files (complete files; in a delta version only what changed, plus `change-manifest.md`):
- `modules/DEMO/P2/db-script-demo.md`
- `modules/DEMO/P2/registry-db-demo.md` (registry)
- Respond with one `<<<FILE: <repo-relative path>>>> … <<<END FILE>>>` block per file when running through a command runner; when running as the operator, write the files directly.
...
| DBF-DEMO-001 | …     | …      | …                    | ENT-DEMO-001.[field] | REQ-DEMO-… | NOT NULL | — |
...
| XM-DEMO-001 | HARD-FK   | …          | [FK column]     | …            | [code]        | REQ-DEMO-… | READY / DEFERRED |
| XM-DEMO-002 | SOFT-READ | (application) | [join / read pattern] | … | [code]  | REQ-DEMO-… | ACTIVE / CONDITIONAL |
...
For every SOFT-READ XM: register it (Type SOFT-READ); add a commentary block:
  -- XM-DEMO-[n] SOFT-READ — this module's [service/query] reads [TARGET].[COLUMN]
...
composite vs surrogate key, an index strategy, a precision):
  ...
  structure) → ADR status BLOCKED, STOP the pass
      action: adr · status: BLOCKED · then: stop
ADR file : decisions/DEMO/ADR-{MOD}-{seq:03d}.md  (Context · Decision · Consequences · traces · status)
Details  : shared/GOVERNANCE-CORE.md
```
...
XM index      : XM id · type · from DEMO · to [code] · status    (→ dependency index)
Lookups       : key · seeded values count · owner
Sequences     : last DBF · last XM
Decisions     : ADR ids (+ BLOCKED, if any)
Event         : "P2 completed: DEMO v1 — [N] tables, [N] DBF, [N] XM"
Cascade       : for every registry XM row targeting DEMO with status DEFERRED, note
...
==============================================================================
# BRIEF — stage `P3.1` (Backend Execution Plan) · module DEMO · v1 · profile `erp`
...
- Read only what this brief contains (generated current state); never open version folders yourself.
- Write exactly these files (complete files; in a delta version only what changed, plus `change-manifest.md`):
- `modules/DEMO/P3_1/backend-execution-plan-demo.md`
- `modules/DEMO/P3_1/registry-exec-be-demo.md` (registry)
- Respond with one `<<<FILE: <repo-relative path>>>> … <<<END FILE>>>` block per file when running through a command runner; when running as the operator, write the files directly.
...
MODULE        : DEMO · v1 · profile erp (ERP Platform)
READS         : srs · db-script · registry-srs · registry-db   (all from _state/ — generated current state)
PRODUCES      : backend-execution-plan-demo.md · registry-exec-be-demo.md
OWNS IDS      : API, QR
...
| `domain/` steering + `profile.knowledge.files` | `profiles/erp/knowledge/erp-domain-standards.md` | primary sources cited when a best-practice choice must be made (§12) |
...
```
PRE-GENERATION EXTRACTION — DEMO v1
── FROM srs ──────────────────────────────────────────────────────────────
ENTITIES      ENT-DEMO-<seq> │ exact name │ kind ∈ master | transactional | lookup | config | security
REQUIREMENTS  REQ-DEMO-<seq> │ EARS text  │ its AC-DEMO-<seq> list (Given/When/Then)
RULES         RULE-DEMO-<seq> │ scope ENT │ trigger │ statement │ message per language (ar, en) │ source
...
COLUMNS       exact column name │ DBF-DEMO-<seq> │ declared type │ null │ default
CONSTRAINTS   exact FK / UNIQUE / CHECK constraint names ; INDEXES exact names
XM            XM-DEMO-<seq> │ kind (HARD-FK | SOFT-READ …) │ local column │ target module.table │ status
── FROM registries ───────────────────────────────────────────────────────
...
```
EXECUTION PLAN INDEX — DEMO v1 — backend-execution-plan-demo.md
Profile: erp · dialect: postgresql16 · framework: profile.stack.backend.framework
Open ADRs: <n> — decisions/DEMO/
...
DBF-DEMO-001 │ ENT-DEMO-001 │ <property>    │ <type>    │ —                            │ ✓
DBF-DEMO-007 │ ENT-DEMO-001 │ <property>    │ <type>    │ XM-DEMO-001 ⏸           │ ⏸
...
- Format: `QR-DEMO-{seq}` (3-digit sequence, continuous across the module).
...
```
QR-DEMO-<seq> — <operation name>
Phase        : <p.key of the phase that defines it>
API          : API-DEMO-<seq> | repository-only
Entity       : ENT-DEMO-<seq>
...
```
### ENT-DEMO-<seq> — <exact name>      kind: <master|transactional|lookup|config|security>
...
```
<!-- API:API-DEMO-<seq>:START traces=REQ-DEMO-<seq>,DBF-DEMO-<seq> -->
### API-DEMO-<seq> — <operation>
...
`api-docs-demo.md` published after implementation, never to this summary.

**R5 — Cross-module consume (contracts).** The plan never mints `XM-*`; it places every XM
from the db-script register:
```
<!-- XM:XM-DEMO-<seq>:START traces=REQ-DEMO-<seq> -->
### XM-DEMO-<seq> — <dependency>
...
DECISIONS         every non-obvious inference is an ADR in decisions/DEMO/ │ no BLOCKED ADR left unsurfaced
...
ADRs             decisions/DEMO/ADR-DEMO-<seq> … (status)
...
```
gov.py split --track backend --module DEMO --version 1 --dry-run
```
...
- **non-breaking** (does not contradict a locked decision or a REQ) → choose the best-practice
  ...
  `decisions/DEMO/ADR-DEMO-{seq:03d}.md` (Context / Decision /
